...
# %matplotlib notebook
# %pylab
from thesis_lib.testdata_generators import model_add_grid, convolved_grid
...
plot(grpd.λ, estimate_fwhm(anisocado)/grpd.snr, ':', label='FWHM/SNR')
plt.legend()
...
plot(grpd.index, estimate_fwhm(anisocado)/grpd.snr, ':', label='FWHM/SNR')
plt.legend()
plt.yscale('log')
...
# %%
from thesis_lib.fitting_weights1D import fit, Gaussian1D, anderson_gauss, anderson, xs, \
fillqueue, anderson_gauss_ramp, plot_lambda_vs_precission_relative
...
res_anderson = fit(model, σ=15, λ=1000, weight_calc=anderson, iters=1, seed_queue=DummySeedQueue(), return_arrays=True)[0]
res_anderson_gauss = fit(model, σ=15, λ=5000, weight_calc=anderson_gauss, iters=1, seed_queue=DummySeedQueue(), return_arrays=True)[0]
...
# %%
figs = plot_lambda_vs_precission_relative(results_pic[(results_pic.σ<20) & (results_pic.weight_calc != 'anderson_gauss_ramp')])
